...
export class EmailService {
  ...
  async sendUpgradeConversion(email: string, name: string) {
    ...
    if (!rdPublicToken) {
      this.logger.warn('Token do RD Station não configurado no .env');
      return;
    }

    const url = `https://api.rd.services/platform/conversions?api_key=${rdPublicToken}`;
    
    const payload = {
      event_type: "CONVERSION",
      event_family: "CDP",
      payload: {
        conversion_identifier: "upgrade_syncro_pro", // O nome exato que colocámos no RD Station
        email: email,
        name: name,
        tags: ["cliente_pro"]
      }
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error(`Erro ao enviar conversão para RD Station: ${response.status}`);
      } else {
        this.logger.log(`✅ Conversão de Upgrade (RD Station) enviada para: ${email}`);
      }
    } catch (error) {
      this.logger.error('Falha na comunicação com a API do RD Station', error);
    }
  }
}